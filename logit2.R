...
df <- read.csv("SP_long_1.csv", header=TRUE)
...
mymodel <- mlogit(Choice~access+DRT_IVT_max+DRT_cost+PM_cost+Bus_cost+Bus_delay+DRT_delay|gender+age+undergrad+r_type+PM_exp|IVT,
                  reflevel="DRT",
                  constPar=c("Bus_cost", "Bus_delay", "DRT_delay"),
                  data=mydata)
...
# fitted
fitted <- data.frame(mymodel$probabilities)
fitted
